...
from flask import url_for, request, redirect, jsonify, make_response
...
def editCategoryItem(catalog_id, item_id):
    ...
    if request.method == 'POST':
        ...
        flash('Catergory Successfully Edited %s' % categoryItem.name)
...
def gconnect():
    ...
    if result['issued_to'] != CLIENT_ID:
            response = make_response(json.
                                     dumps("Token's client ID does not"
                                           "match app's."), 401)
...
def gdisconnect():
    ...
    if result['status'] != '200':
        ...
        response = make_response(json.
                                 dumps('Failed to revoke token for'
                                       'given user.'), 400)
        response.headers['Content-Type'] = 'application/json'
        return response
